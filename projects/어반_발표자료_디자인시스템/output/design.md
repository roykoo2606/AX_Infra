...
#### 강조색은 퍼플이며 테마마다 다른 값을 쓴다 (2026-07-27 개정)

강조는 브랜드 상징색인 퍼플로 한다. **하나의 값을 두 테마에 공용하지 않는다.**

| 테마 | 토큰 | 값 | 채도 | 배경 대비 |
|---|---|---|---|---|
| dark | `--accent` | `#9B8AF2` | S43% | 6.8:1 |
| light | `--accent` | `#5F45D6` | S68% | 6.3:1 |

- **"톤다운"은 채도를 낮추는 것이지 어둡게 하는 것이 아니다.** 어두운 배경에서는 채도를 낮추면서
  명도를 올려야 눈이 편하면서도 읽힌다. 반대로 흰 배경에서는 명도를 낮춘다.
- 폐기: `#00FFCB`. 채도·명도가 모두 최대라 다크에서 대비 14.9:1로 눈을 찌르고,
  라이트에서는 1.3:1로 보이지 않았다. 어느 테마에서도 쓰지 않는다.
- 민트 `#14DEAF`는 로고·브랜드 그라디언트에만 남긴다. 표 헤더·단위 등 반복 요소의 강조에는 쓰지 않는다
  (라이트 배경 대비 1.7:1로 읽히지 않는다).
- 강조색을 쓰는 곳은 장표당 **한 종류의 의미**로 제한한다. 이 예시에서는 "병목 지표"뿐이다.
...
`--accent`는 포커스 점, 선택 상태, 핵심 수치 밑줄에만 쓴다. 넓은 배경에는 쓰지 않는다.
데이터 밀도가 높은 장표는 색을 쓸 면적 자체가 적으므로 위 비율의 하한만 지키면 된다.
